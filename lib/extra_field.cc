...
const zip_uint8_t* ExtraField::createPkWareUnixField(time_t mtime,
                                                     time_t atime,
                                                     mode_t mode,
                                                     uid_t uid,
                                                     gid_t gid,
...
const zip_uint8_t* ExtraField::createInfoZipNewUnixField(uid_t uid,
                                                         gid_t gid,
                                                         zip_uint16_t& len) {
...
inline static uint64_t timespec2ntfs(const timespec& ts) {
  return static_cast<uint64_t>(ts.tv_sec) * 10000000 +
         static_cast<uint64_t>(ts.tv_nsec) / 100 + NTFS_TO_UNIX_OFFSET;
}

const zip_uint8_t* ExtraField::createNtfsExtraField(const timespec& mtime,
...
zip_uint16_t ExtraField::editNtfsExtraField(zip_uint16_t len,
                                            zip_uint8_t* data,
